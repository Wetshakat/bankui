document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleTransaction(type) {
        let amountInput = prompt(`Enter amount to ${type}:`);
        if (!amountInput) return;
        let amount = parseFloat(amountInput.trim());
    ...
    function transferMoney() {
        let recipientAccNumberInput = prompt("Enter recipient's account number:");
        if (!recipientAccNumberInput) return;
        let recipientAccNumber = recipientAccNumberInput.trim();
        ...
        users = users.map(user => 
            user.accountNumber === loggedInUser.accountNumber ? { ...loggedInUser, balance } :
            user.accountNumber === recipient.accountNumber ? { ...recipient } : user
        );
